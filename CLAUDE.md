...
11. **Kitchen count roundoff rule:** ALL kitchen summary counts and kg portions (Dal, Dal Fry, etc.) use `_customKitchenRound` (decimal $\ge 0.35$ rounds up $+1$, $\le 0.34$ rounds down $+0$). e.g., Varan `11.97` $\rightarrow$ `12`, Dal Fry `7.98` $\rightarrow$ `8`.
12. **Negative Wallet Balances:** If a user's wallet has a negative balance (e.g. manual admin deduction), the system automatically forces them to pay it off on their next order (standard or bulk) by inflating the checkout/gateway total and logging a `Debt Recovery Recharge` to reset the wallet to 0.
