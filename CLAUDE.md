...
5. Delivery: main site ₹11/meal, free at ₹106/159/190, free areas Bhosale Nagar+Triveni Nagar+pickup/porter. LS: always free. Cap bypass ≥₹200 (breakfast ₹100). **Cap Counting:** unique customer name OR exact identical address (wing+flat+society) per meal = 1 slot; **Magarpatta Cybercity Towers 1–12:** all orders at the same tower collapse to 1 slot via `towerKey = "mpt|N"` (regex `tower\s*(\d{1,2})` on Society+Wing+Flat+Area, Amanora excluded); VIPs (Fee_Exempt) = 0 slots; Enkin/IA collapse to 1; Shree Laxmi Vihar, Momstory Hospital (desk drop) & Metapercept = 0 slots; LS = 0 slots, never blocked.
...
- **Delivery Slot Status & Attribution on Admin Orders Tab (CODE_VERSION 35.51 / ADMIN_VERSION v26.09.08.08)**
  ...
  - **Exempt Reasons:** Identifies exact exemption reasons (`Self Pickup`, `Porter Courier`, `Metapercept`, `Momstory Desk Drop`, `Shree Laxmi Vihar Home Base`, `VIP Customer`, `Liviano-Serio`).
...
- **Admin Wallet Debit & Negative Balance Provision (CODE_VERSION 35.74 / APP_VERSION v26.09.25.01 / ADMIN_VERSION v26.09.25.01)**
  - **Negative Balance & Wallet Debit Support:** `adminCreditWallet()` in `04_Reports_Misc.gs` now accepts negative numbers (e.g. `-234`) to debit a customer's wallet. It logs the transaction as `Txn_Type: "Admin Debit"` with absolute amount, deducts the amount from wallet balance, skips `_autoSettlePendingOrders` (debits never auto-settle pending orders), and returns an informative confirmation message (`₹234 debited from <phone>. New balance: ₹<bal>`).
  - **Ledger & Balance Math Hardening:** `_calculateWalletBalance()` in `02_Orders_Menu.gs` now includes `rType.includes("debit")` in deduction branches using `Math.abs(rAmt)`. `getWalletTransactions()` returns `direction: "debit"` when `rAmt < 0` or debit type, preventing broken `+₹-138` displays on historical negative rows.
  - **Admin Panel UI (`docs/Admin/vault_admin.html`):** Removed `min="1"` limitation from `#wcAmount`, added `step="any"` and hint `(use - to debit)`, updated validation in `adminAddWalletCredit()` to allow any non-zero amount, and updated status indicators to show debit vs credit styling.
  - **Frontend Transaction Modal Sync:** Added `"admin debit"` (`🔻`) to `TYPE_LABEL` in `docs/order.html` and `docs/Liviano-Serio.html`.
- **Delivery Route Ranking Optimization (CODE_VERSION 35.73 / APP_VERSION v26.09.24.02)**
  - **Confidence-Weighted Ranking:** `buildDeliveryRoute()` in `03_Admin_Kitchen.gs` now uses a blended position formula: `blended = confidence × median + (1 − confidence) × 0.5` where `confidence = min(samples / CONFIDENCE_THRESHOLD, 1.0)` and `CONFIDENCE_THRESHOLD = 5`. Societies with few data points regress toward mid-route instead of dominating top/bottom positions (fixes e.g. Advocate Jadhav with 1 sample at position 0 being ranked #3).
  - **Canonical Society Display Names:** Route tab society names now resolve through `_getCanonicalSocietyDisplay()` → `DISPLAY_TITLES` map first, merging duplicates like "Grevillea"/"Grevillia" into a single canonical row. Falls back to most-used raw spelling only for unmapped societies.
  - **Self-Pickup Exclusion:** Filters out `selfpickup` / `self pickup` entries from route tabs since they are not delivery stops.
  - **Applies to All Meals:** Changes apply to Breakfast, Lunch, and Dinner route tabs via the existing `ROUTE_MEALS.forEach()` loop.
- **Metapercept Slot-Free Exemption (CODE_VERSION 35.72 / APP_VERSION v26.09.24.01)**
  - **Zero Delivery Slot Consumption:** Orders for Metapercept (Phone `7517913755`, customer name `Metapercept`, or address containing `metapercept`) are slot-free, consuming 0 delivery slots towards the daily kitchen delivery cap.
  - **Admin Orders Tab Attribution:** `getOrderSummary` in `03_Admin_Kitchen.gs` attributes `slotType: "exempt"` with `slotReason: "Metapercept"`, rendering as `⚪ Metapercept` badge instead of `🟢 Slot #N`.
  - **Cap Exemption & Non-Blocking:** Synchronized across backend (`_countActiveMealOrders` and `_submitOrderInternal` cap guard in `02_Orders_Menu.gs`), admin dashboard stats (`_getAdminDataUncached` in `03_Admin_Kitchen.gs`), and customer frontend (`_mealKeepsDeliveryAtCap` in `docs/order.html` & `docs/Liviano-Serio.html`).
  - **Kitchen Prep Retained:** Items and food quantities continue to count normally toward kitchen prep totals, labels, and driver packaging.
- **Food Clarity, Nutrition Transparency, Volume vs Weight & Oils (CODE_VERSION 35.71 / APP_VERSION v26.09.16.08)**
  - **1-Tap Food Clarity Across 45+ Catalog Items:** Interactive nutritional and ingredient modal deployed across all 45+ items (Breakfast, Breads, Dry Sabjis, Curry Sabjis, Dal, Rice, Extras) on `docs/order.html` and `docs/Liviano-Serio.html`. Displays approximate calories, protein, carbs, fats, fiber, complete ingredient list, chef kitchen notes, and dietary tags.
  - **Portion & Container Sizing Clarity (Volume vs Weight):** Explicitly calibrated that 100ml (Mini) and 250ml (Full) sabji portions and 200ml dal portions represent food-grade container liquid volumetric capacity (water equivalent); physical weight in grams fluctuates naturally depending on vegetable density and gravy style. Phulka and Ghee Phulka dimensions standardized to 5–5.5 inches diameter.
  - **Cooking Standards & Oil Transparency:** Refined Groundnut Oil for sabjis, Sunflower Oil for chapatis/parathas/frying, Pure Cow Ghee for ghee phulkas and breakfast, Zero Oil for diet items.
  - **Omnichannel Sync:** Synced across `Backend/business.json`, `BUSINESS_CONTEXT` (`00_Config.gs`), `buildSystemPrompt` (`04_Reports_Misc.gs`), `docs/order-chat.js`, Guides & FAQs in `docs/order.html` and `docs/Liviano-Serio.html`, and FAQ accordion + JSON-LD structured data in `docs/index.html`.
- **Upcoming Menu 1-Month Window, Server Caching & Mobile Gestures (CODE_VERSION 35.70 / APP_VERSION v26.09.15.02)**
  - **1-Month Window:** `getWeeklyMenu()` in `02_Orders_Menu.gs` now bounds future dates to a 30-day window (`today <= date <= today + 30d`) instead of 131 days, reducing payload size by >75%.
  - **Server-Side Cache with Invalidation:** Added `_cachedData("weekly_menu_1m_" + todayStr, 900, ...)` caching responses in Apps Script CacheService (TTL 15 min). Invalidated on `saveMenu` and `setKitchenClosed` in `03_Admin_Kitchen.gs`.
  - **Zero-Latency Client-Side Prefetch & Session Caching:** `order.html`, `Liviano-Serio.html`, and `intentamplify.html` prefetch the weekly menu 3s after boot and cache in `sessionStorage` (`svaadh_wm_1m_<ISO>`), allowing instant 0ms opens on tap.
