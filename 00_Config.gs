...
// CAPPED MEAL's food subtotal is ≥ this amount still gets delivery (big orders
// are worth the slot). Applies only when pickup/porter alternatives are ON —
// a cap_alt=false HARD close (kitchen out of capacity) is never bypassed.
// MUST match order.html's CAP_BYPASS_MIN.
const CAP_DELIVERY_BYPASS_MIN = 200;
