...
const CODE_VERSION   = 25.3; // 2026-07-06: On-Account settlement fixes — (1) unified status filter: verifyLogin's displayed due amount, _autoSettlePendingOrders, _computeOnAccountDue, getOnAccountBill, and the admin billing bucket all previously used slightly different ad-hoc filters (some counted legacy "Pending"/blank rows, inflating the customer-visible due without those rows ever being chargeable) — now all route through one _isOnAccountDueStatus("On Account" exact match, the only string ever actually written). (2) FIXED the ₹812/"3 orders skipped" incident's root cause in _settleOnAccountDirect: the old oldest-first loop did `remaining -= net` per row and `break`-ed the WHOLE loop the instant one row's net exceeded what was left, silently abandoning every row after it even if smaller/affordable. Now: a FAST PATH settles every on-account row directly whenever the charge covers the full current balance (the normal/expected case — makes this bug class structurally impossible); a FALLBACK (charge < current balance, e.g. a new on-account order landed mid-payment) skips-and-continues instead of breaking, and emails the owner since the balance shifted mid-payment. Added read-only auditOnAccountStatusDrift() (+?action=auditOnAccountDrift, admin PIN) to list any stray legacy rows in live data. Node-simulated the exact stranding pattern against both old and new logic to confirm. (25.2 below: address hygiene.)
