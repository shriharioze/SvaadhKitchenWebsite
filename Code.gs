...
if (action === "fixCustomerPins") { if (!isAdmin) return jsonRes({ error: "STRICT ADMIN PIN REQUIRED" }); return jsonRes(fixCustomerPins(p.commit === "1")); } // scan + fix PINs that lost leading zeros (dry-run unless commit=1)
    ...
    if (action === "suspendArchive") { if (!isAdmin) return jsonRes({ error: "STRICT ADMIN PIN REQUIRED" }); PropertiesService.getScriptProperties().setProperty("ARCHIVE_SUSPENDED", "true"); return jsonRes({status: "suspended"}); }
    ...
    if (action === "seedCanonicalSocietyAliases") { if (!isAdmin) return jsonRes({ error: "STRICT ADMIN PIN REQUIRED" }); return jsonRes(seedCanonicalSocietyAliases(p.commit === "1")); }
    if (action === "standardizeCustomerAddresses") { if (!isAdmin) return jsonRes({ error: "STRICT ADMIN PIN REQUIRED" }); return jsonRes(standardizeCustomerAddresses(p.commit === "1")); }
...
function doPost(e) {
  ...
  _upsertCustomer(getSpreadsheet(), profile, _lsStorefront(body));
    ...
    if (action === "unmarkDelivered") {
      if (!isStaff) return jsonRes({error:"STRICT STAFF PIN REQUIRED"});
      return jsonRes(unmarkDelivered(body));
    }
